...
var UsersController = class {
  async create(request2, response) {
    const bodySchema = import_zod.z.object({
      ...
      password: import_zod.z.string().min(7)
    });
...
var SessionsController = class {
  async create(request2, response) {
    const bodySchema = import_zod3.z.object({
      ...
      password: import_zod3.z.string().min(7)
    });
...
var DeliveryLogsController = class {
  async create(request2, response) {
    ...
    return response.status(201).json({ message: "Create Delivery Logs" });
  }
...
function errorHandling(error, request2, response, nest) {
